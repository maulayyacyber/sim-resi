...
class Breadcrumb {
	 ...
	 */
	private $_divider 		= '';
	private $_tag_open 		= '<ul class="breadcrumbs-alt">';
	 ...
	 * @return	void
	 */		
	function append_crumb($class, $title, $href)
	{
		// no title or href provided
		if (!$title OR !$href) return;
		
		// add to end
		$this->breadcrumbs[] = array('class' => $class,'title' => $title, 'href' => $href);
	}
	 ...
	 * @return	void
	 */		
	function prepend_crumb($class, $title, $href)
	{
		// no title or href provided
		if (!$title OR !$href) return;
		
		// add to start
		array_unshift($this->breadcrumbs, array('class' => $class,'title' => $title, 'href' => $href));
	}
		...
		if ($this->breadcrumbs) {
			...
			foreach ($this->breadcrumbs as $key => $crumb) {
				...
				if (end(array_keys($this->breadcrumbs)) == $key) {
					$output .= ' <li><a class="'.$crumb['class'].'" href="' . $crumb['href'] . '">' . $crumb['title'] . '</a> </li>';
